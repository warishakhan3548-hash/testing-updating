...
const Color lightCanvas = Color(0xFFF3F5F8);
...
abstract final class UIConstants {
  ...
  static const Duration routePressHandoff = Duration(milliseconds: 56);
...
abstract final class AppStyles {
  ...
  static List<BoxShadow> surfaceDepth(BuildContext context) {
    // PREMIUM_SOFT_3D_V4
    //
    // Crisp contact depth + broad grounding depth.
    // The card reads as a real raised ceramic slab without
    // stacking excessive expensive blur layers.
    final bool dark = isDark(context);
    ...
    return <BoxShadow>[
      BoxShadow(
        color: Colors.white.withAlpha(dark ? 12 : 248),
      ...
      BoxShadow(
        color: ink.withAlpha(dark ? 102 : 38),
        blurRadius: 3.5,
        spreadRadius: -1,
      ...
      BoxShadow(
        color: ink.withAlpha(dark ? 70 : 24),
    ...
    return <BoxShadow>[
      BoxShadow(
        color: ambient.withAlpha(
          _perceptualAlpha(
            ambient,
            strong ? (dark ? 58 : 48) : (dark ? 45 : 35),
          ),
        ),
        blurRadius: strong ? 24 : 19,
      ...
      if (strong)
        BoxShadow(
          color: ambient.withAlpha(_perceptualAlpha(ambient, dark ? 25 : 20)),
          blurRadius: 38,
          spreadRadius: -12,
  ...
  static List<BoxShadow> railDepth(BuildContext context, Color color) {
    ...
    return <BoxShadow>[
      BoxShadow(
        color: ambient.withAlpha(_perceptualAlpha(ambient, dark ? 52 : 40)),
        blurRadius: 21,
        spreadRadius: -5,
  ...
  static Border glassBorder(BuildContext context, {Color? accent}) {
    final bool dark = isDark(context);
    final Color neutral = Colors.white.withAlpha(dark ? 28 : 92);
    final Color resolved = accent == null
        ? neutral
        : Color.alphaBlend(accent.withAlpha(dark ? 18 : 12), neutral);
    return Border.all(color: resolved, width: UIConstants.borderWidth);
  }
...
class _GlobalTapRippleLayerState extends State<_GlobalTapRippleLayer>
  ...
  static const double _scrollThreshold = kTouchSlop;
  ...
  void _startPulse(_RipplePulse pulse) {
    if (!mounted || !_livePulses.contains(pulse) || !pulse.prepareStart()) {
      return;
    }
  ...
  void _handlePointerDown(PointerDownEvent event) {
    ...
    // Immediate finger-down depth is owned by _Pressable.
    // The global ripple is committed only on pointer-up, so a drag/scroll
    // never launches a full-screen paint animation that is cancelled later.
  }
...
class _OpaqueContentTransitionsBuilder extends PageTransitionsBuilder {
  ...
  Widget buildTransitions<T>(
    ...
    // Keep the route canvas fully opaque so transparent Scaffolds never blend
    // two pages. Motion is compositor-only; resting visuals remain unchanged.
    if (AppMotion.reduce(context)) {
      return _AmbientBackground(child: child);
    }

    final Animation<double> contentMotion = CurvedAnimation(
      parent: animation,
      curve: UIConstants.motionOut,
      reverseCurve: UIConstants.motionIn,
    );

    final Animation<double> contentOpacity = Tween<double>(
      begin: .96,
      end: 1,
    ...
    final Animation<double> contentScale = Tween<double>(
      begin: .992,
      end: 1,
    ).animate(contentMotion);
...
class _FastRouteLauncherState extends State<_FastRouteLauncher> {
  ...
  void _open() {
    ...
    void launchRoute() {
      ...
      // The handoff is intentionally delayed for tactile release. If another
      // route won during that small window, never let this stale tap push over it.
    ...
    if (AppMotion.reduce(context)) {
      ...
      _launchTimer = Timer(UIConstants.routePressHandoff, launchRoute);
    }
  }
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                      ...
                      colors: <Color>[Color(0xF01C2230), Color(0xE6121214)],
                    ),
                    ...
                    boxShadow: <BoxShadow>[
                      ...
                      BoxShadow(color: appleBlue.withAlpha(20), blurRadius: 34),
                    ],
                  ),
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
...
class _AppShellState extends State<AppShell> with WidgetsBindingObserver {
  ...
  late final ValueNotifier<List<Color>> _tabColors;
  ...
  void _handleTabColorSync() {
    ...
    _tabColors.value = List<Color>.unmodifiable(next);
  }
  ...
  Duration _adaptivePageDuration(double originPage, int targetPage) {
    ...
    // Preserve live-finger retarget responsiveness while allowing distant
    // tab selections to travel continuously instead of teleporting to an
    // artificial staging page. Square-root timing keeps long travel calm.
  ...
  void _selectTab(int index) {
    ...
    // A text field on the outgoing kept-alive page must not own the keyboard
    // or focus after the user deliberately changes modules.
    ...
    final double livePage =
        _pageController.page ?? _settledPageIndex.toDouble();
    final Duration pageDuration = _adaptivePageDuration(livePage, index);
  ...
  bool _handlePageScrollNotification(ScrollNotification notification) {
    ...
    if (notification is ScrollStartNotification &&
      ...
      // Horizontal module navigation is a clear intent to leave the focused
      // field; dismiss it before the next page becomes visible.
      FocusManager.instance.primaryFocus?.unfocus();
  ...
  Widget _buildSwipeMotionPage(int index, Widget child) => AnimatedBuilder(
    ...
    builder: (BuildContext context, Widget? stableChild) {
      ...
      final double rawDelta = page - index;
      // Kept-alive pages more than one viewport away are invisible. Do not
      // allocate transform/compositing work for them on every pager tick.
      if (rawDelta.abs() > 1.05) return pageChild;
      final double delta = rawDelta.clamp(-1.0, 1.0).toDouble();
...
class _ActiveSyncViewState extends State<_ActiveSyncView> {
  ...
  void _handleActivePageChange() {
    ...
    // renders the latest sync state exactly once.
    if (mounted) setState(() {});
  }
...
class _BottomLedgerNav extends StatelessWidget {
  ...
  Widget _buildNavigation(BuildContext context) {
    ...
    final int safeSelected = colors.isEmpty
        ? 0
        : selected.clamp(0, colors.length - 1).toInt();

    final Color activeColor = colors.isEmpty ? appleBlue : colors[safeSelected];
    ...
    return Padding(
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          ...
          boxShadow: <BoxShadow>[
            BoxShadow(
              color: activeColor.withAlpha(dark ? 34 : 22),
              blurRadius: 25,
              spreadRadius: -8,
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: dark
                    ...
                    : const <Color>[Color(0xFFFFFFFF), Color(0xFFF0F4F8)],
              ),
                    ...
                    child: Row(
                      children: List<Widget>.generate(_tabs.length, (
                        ...
                        return _Pressable(
                          ...
                          // Reselecting the active destination changes no state,
                          // so do not emit a false selection-change haptic.
                          haptics: !active,
    ...
    decoration: BoxDecoration(
      ...
      gradient: LinearGradient(
        ...
        colors: active
            ...
            : <Color>[color.withAlpha(dark ? 15 : 8), Colors.transparent],
      ),

      border: Border.all(
        color: color.withAlpha(active ? (dark ? 82 : 66) : (dark ? 24 : 18)),
        width: active ? .9 : .7,
      ),

      boxShadow: active ? AppStyles.jewelDepth(context, color) : null,
    ),
...
class _PressableState extends State<_Pressable>
  ...
  late final ChangeNotifier _spatialRepaint;
  late final Listenable _pressRepaint;
  ...
  void initState() {
    ...
    _spatialRepaint = ChangeNotifier();

    _pressRepaint = Listenable.merge(<Listenable>[
      _pressController,
      _spatialRepaint,
    ]);
  }
  ...
  bool _updateTouchAlignment(Offset localPosition) {
    ...
    final double x = (localPosition.dx / extent.width * 2 - 1)
        .clamp(-.72, .72)
        .toDouble();
  ...
  void _sampleGestureVelocity(Offset localPosition) {
    ...
    final bool reversing =
        previousSpeed > 80 && sampleSpeed > 80 && directionAgreement < -.20;
  ...
  Offset _freshGestureVelocity() {
    ...
    final double speedIntent = (_gestureVelocity.distance / 1800)
        .clamp(0.0, 1.0)
        .toDouble();
    ...
    final double freshness = 1 - (ageMicros / adaptiveFreshnessWindowMicros);
  ...
  void _trackTouch(TapMoveDetails details) {
    ...
    if (changed && !_pressController.isAnimating) {
      _spatialRepaint.notifyListeners();
    }
  }
  ...
  Widget build(BuildContext context) => Semantics(
    ...
    child: GestureDetector(
      ...
      child: AnimatedBuilder(
        animation: _pressRepaint,
        child: widget.child,
        builder: (BuildContext context, Widget? child) {
          final double feedback = _pressController.value;
          // Zero is the overwhelmingly common list state. Skip identity
          // transforms, transparent glow layers and matrix work entirely.
          if (feedback.abs() <= .0001) return child!;
          final bool dark = Theme.of(context).brightness == Brightness.dark;
          final bool reduceMotion = AppMotion.reduce(context);
...
class _GlassCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (borderColor != null) {
      ...
      resolvedBorder = dark
          ? Color.alphaBlend(semantic.withAlpha(64), Colors.white.withAlpha(39))
          : Color.alphaBlend(semantic.withAlpha(17), const Color(0xF0FFFFFF));
    }
    ...
    if (dark) {
      ...
      shadows = <BoxShadow>[
        ...
        if (ambient != null && tintColor != null)
          BoxShadow(
            color: ambient.withAlpha(AppStyles._perceptualAlpha(ambient, 21)),
            blurRadius: 42,
            spreadRadius: -13,
            ...
            gradient: LinearGradient(
              ...
              colors: dark
                  ...
                  : <Color>[
                      ...
                      Color.lerp(const Color(0xFFF0F4F8), color, .05)!,
                    ],
            ),

            border: Border.all(
              color: color.withAlpha(dark ? 84 : 62),
              width: UIConstants.borderWidth,
            ),
        ...
        decoration: BoxDecoration(
          ...
          gradient: dark
              ...
              : const LinearGradient(
                  ...
                  colors: <Color>[Color(0xFFFFFFFF), Color(0xFFF0F4F8)],
                ),

          border: Border.all(
            color: appleBlue.withAlpha(dark ? 62 : 34),
            width: UIConstants.borderWidth,
          ),

          boxShadow: AppStyles.jewelDepth(context, appleBlue),
        ),

        child: const Icon(Icons.chevron_left_rounded, color: appleBlue),
      ),
    );
  }
}
...
class _SearchBoxState extends State<_SearchBox>
    with AutomaticKeepAliveClientMixin<_SearchBox> {
  Timer? _debounce;
  ...
  Widget build(BuildContext context) {
    ...
    final Color edge = focused
        ? widget.color.withAlpha(dark ? 138 : 104)
        : Color.alphaBlend(
    ...
    return AnimatedContainer(
      duration: AppMotion.reduce(context) || themeBrightnessChanged
          ? Duration.zero
          : const Duration(milliseconds: 180),
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: edge, width: focused ? 1.25 : .8),

        boxShadow: focused
            ? AppStyles.glow(context, widget.color, strong: true)
            : AppStyles.glow(context, widget.color),
      ),
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle: TextStyle(
            color: widget.color.withAlpha(dark ? 176 : 164),
            fontSize: 16.5,
            fontWeight: FontWeight.w600,
          ),
          ...
          prefixIcon: Icon(
            ...
            color: widget.color,
            size: 25,
            shadows: AppStyles.inkGlow(widget.color, strong: true),
          ),
...
class _LazyLedgerList extends StatelessWidget {
  const _LazyLedgerList({
  ...
  Widget build(BuildContext context) {
    ...
    final int contentCount = itemCount == 0 ? 1 : itemCount;
...
class _AmountHero extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool dark = Theme.of(context).brightness == Brightness.dark;
    final bool hasSecondary = secondaryLabel != null && secondaryValue != null;
          ...
          children: <Widget>[
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: color.withAlpha(dark ? 102 : 84),
                  width: UIConstants.borderWidth,
                ),
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      color: color.withAlpha(dark ? 205 : 188),
                      fontSize: 13.5,
                      height: 1.15,
                      fontWeight: FontWeight.w700,
                    ),
                    ...
                    : Column(
                        ...
                        children: <Widget>[
                          Text(
                            ...
                            textAlign: TextAlign.right,
                            style: TextStyle(
                              color: color.withAlpha(dark ? 218 : 198),
                              fontSize: 10.5,
                              height: 1,
                              fontWeight: FontWeight.w800,
                              letterSpacing: .68,
            ...
            if (onDelete != null)
              ...
              Padding(
                ...
                child: Icon(
                  ...
                  color: color.withAlpha(dark ? 190 : 166),
...
void _popIfCurrent<T>(BuildContext context, [T? result]) {
...
class _SheetDisposalBarrier extends StatefulWidget {
  ...
  @override
  State<_SheetDisposalBarrier> createState() => _SheetDisposalBarrierState();
}

class _SheetDisposalBarrierState extends State<_SheetDisposalBarrier> {
  @override
...
Future<T?> _openSheet<T>(BuildContext context, Widget child) async {
  final bool reduceMotion = AppMotion.reduce(context);
  ...
  final T? result = await showModalBottomSheet<T>(
    ...
    builder: (BuildContext sheetContext) => _SheetDisposalBarrier(
      onDisposed: () {
  ...
  if (!disposed.isCompleted) await disposed.future;
    ...
    builder: (BuildContext context) => AlertDialog(
      ...
      actions: <Widget>[
        _BounceTextButton(
          ...
          onTap: () => _popIfCurrent(context, false),
    ...
    return _Pressable(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: AppStyles.jewelDepth(context, const Color(0xFF7957E8)),
        ),
        child: Stack(
          fit: StackFit.expand,
          children: <Widget>[
      ...
      children: <Widget>[
        ...
        Expanded(
          child: RefreshIndicator.adaptive(
            onRefresh: () async {
              HapticFeedback.lightImpact();
              final bool synced = await sync.reconcile(
              ...
              if (synced) {
                ...
                _toast(
                  context,
                  'Sync is incomplete. Check your Firebase connection and retry.',
                  error: true,
                );
              }
            },
...
class _LedgerIcon extends StatelessWidget {
  const _LedgerIcon({required this.icon, required this.color, this.size = 48});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: color.withAlpha(dark ? 104 : 86),
          width: UIConstants.borderWidth,
        ),
...
class _MilkScreenState extends State<MilkScreen> {
  ...
  Future<void> _addCustomer() async {
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: _TransactionPairButton(
                    ...
                    onTap: () {
                      _popIfCurrent(sheetContext, 'dene_wala');
                    },
                  ),
                ),
              ],
    ...
    if (duplicate) {
      _toast(
        context,
        'Customer already exists — duplicate not created.',
        error: true,
      );
      return;
    }
              ...
              children: <Widget>[
                Expanded(
                  child: _TransactionPairButton(
                    ...
                    onTap: () {
                      _popIfCurrent(sheetContext, 'given');
                    },
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: _TransactionPairButton(
                    ...
                    onTap: () {
                      _popIfCurrent(sheetContext, 'taken');
                    },
                  ),
                ),
              ],
...
class _SoftShareButton extends StatelessWidget {
...
class _LedgerDeleteAction extends StatelessWidget {
  const _LedgerDeleteAction({required this.onTap, required this.semanticLabel});
...
class _LedgerAmountCell extends StatelessWidget {
  const _LedgerAmountCell({required this.value, required this.color});
...
class _LedgerBadgeCell extends StatelessWidget {
  const _LedgerBadgeCell({required this.label, required this.color});
...
class _SalaryScreenState extends State<SalaryScreen> {
  ...
  Future<void> _addPerson() async {
    ...
    if (database.keys.any(
      (String key) => key.toLowerCase() == personName.toLowerCase(),
    )) {
    ...
    final bool? save = await _openSheet<bool>(
      ...
      _SheetFrame(
        ...
        children: <Widget>[
          ...
          Builder(
            builder: (BuildContext sheetContext) => _PrimaryButton(
              label: 'Save Salary',
              color: tone,
              onTap: () => _popIfCurrent(sheetContext, true),
            ),
          ),
        ],
      ),
...
class _CreditScreenState extends State<CreditScreen> {
  ...
  Future<void> _addEntry({String? fixedName}) async {
                  ...
                  children: <Widget>[
                    Expanded(
                      child: _TransactionPairButton(
                        ...
                        onTap: () {
                          type = 'credit';
                          _popIfCurrent(sheetContext, true);
                        },
                      ),
                    ),
                    const SizedBox(width: 12),
    ...
    if (LedgerMath.strictDate(entryDate) == null ||
      ...
      _toast(
        context,
        'Fill the complete form with a valid amount.',
        error: true,
      );
      return;
    }
      ...
      children: <Widget>[
        ...
        Expanded(
          child: _LazyLedgerList(
            ...
            itemBuilder: (BuildContext context, int index) {
              final CreditPartySummary group = groups[index];
              ...
              children: <Widget>[
                Expanded(
                  child: _TransactionPairButton(
                    ...
                    onTap: () {
                      _popIfCurrent(sheetContext, 'credit');
                    },
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: _TransactionPairButton(
                    ...
                    onTap: () {
                      _popIfCurrent(sheetContext, 'debit');
                    },
                  ),
                ),
              ],
      ...
      children: <Widget>[
        ...
        Expanded(
          child: _LazyLedgerList(
            ...
            itemBuilder: (BuildContext context, int index) {
              final ExpenseCategorySummary group = groups[index];
...
class _DiaryScreenState extends State<DiaryScreen> {
  ...
  Future<void> _selectPeriod(int month, int year) async {
    ...
    try {
      ...
      if (!mounted) return;
      // DIARY_STALE_SELECTION_GUARD_V2
      // An older failed month load must not show an error over a newer
      // successful selection, and must not roll that newer selection back.
...
class _BusinessScreenState extends State<BusinessScreen> {
  ...
  Future<void> _addProject() async {
    ...
    if (database.keys.any(
      (String key) => key.toLowerCase() == project.toLowerCase(),
    )) {
...
class _BusinessDetailScreenState extends State<BusinessDetailScreen> {
  ...
  Future<void> _addEntry(BuildContext context) async {
    ...
    if (LedgerMath.strictDate(entryDate) == null || entryAmount <= 0) {
      _toast(
        context,
        'Enter details and a valid positive amount.',
        error: true,
      );
      return;
    }
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (BuildContext context, Widget? child) {
      ...
      final List<Map<String, dynamic>> allRecords = _rows(project['records']);
...
class _AiHubScreenState extends State<AiHubScreen> with WidgetsBindingObserver {
  ...
  String _apiKeyStorageKey(String ownerUid) =>
      _scopedSetting('gemini.apiKey', ownerUid);

  String _modelSettingKey(String ownerUid) =>
      _scopedSetting('gemini.model', ownerUid);
  ...
  Future<void> _loadSettings() async {
    ...
    String key = '';
    try {
      key = await _secureStorage.read(key: _apiKeyStorageKey(ownerUid)) ?? '';
    } catch (error, stackTrace) {
      debugPrint('AI secure storage read failed: $error\n$stackTrace');
    }
    if (!mounted || _currentOwnerUid != ownerUid) return;
    final String model = _normalizeGeminiModel(
      widget.sync.readSetting(_modelSettingKey(ownerUid)),
    );
    ...
    final bool batchAlreadyCompleted =
        batchJob != null &&
        completedFingerprints.contains(batchJob.fingerprint);
  ...
  Future<void> _configure() async {
    ...
    try {
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: _BounceIconButton(
                        ...
                        onTap: () => setSheetState(() => obscure = !obscure),
                        icon: Icon(
                          obscure
      ...
      if (save != true || !mounted || _currentOwnerUid != ownerUid) return;
      final String cleanKey = key.text.trim();
      try {
        await _secureStorage.write(
          key: _apiKeyStorageKey(ownerUid),
          value: cleanKey,
        );
        await widget.sync.writeSetting(_modelSettingKey(ownerUid), model);
      } catch (error, stackTrace) {
        debugPrint('AI setup save failed: $error\n$stackTrace');
        if (mounted && _currentOwnerUid == ownerUid) {
          _toast(context, 'AI setup could not be saved.', error: true);
        }
        return;
      }
      if (!mounted || _currentOwnerUid != ownerUid) return;
  ...
  Future<void> _shareExternalAiPackage(BuildContext sheetContext) async {
    ...
    try {
      ...
      if (!mounted || !session.isCurrent) return;
      if (sheetContext.mounted) _popIfCurrent(sheetContext);
      if (!mounted || !session.isCurrent) return;
      setState(() {
  ...
  Future<void> _cancelRemainingBatch() async {
    ...
    try {
      ...
      debugPrint('AI batch cancel persistence failed: $error\n$stackTrace');
      if (_isOwnerSessionCurrent(job.ownerUid, ownerGeneration)) {
        _toast(
          context,
          'AI batch could not be cancelled. Please retry.',
          error: true,
        );
        _armBatchTimers();
      }
      ...
      builder: (BuildContext dialogContext) => AlertDialog(
        ...
        actions: <Widget>[
          _BounceTextButton(
            ...
            onTap: () => _popIfCurrent(dialogContext, false),
            child: const Text('रद्द करें'),
          ),
          _BounceTextButton(
            semanticLabel: '${plan.actions.length} बदलाव लागू करें',
            onTap: () => _popIfCurrent(dialogContext, true),
            child: Text(
  ...
  static Future<_AiOutcome> generate({
    ...
    if (finishReason == 'MAX_TOKENS') {
      throw const LedgerSyncException(
        'Gemini response was cut off at the output limit. Split the request '
        'into smaller groups and retry.',
      );
    }
    ...
    if (finishReason.isNotEmpty &&
      ...
      throw LedgerSyncException(
        ...
        '${finishMessage.isEmpty ? '.' : ': $finishMessage'}',
      );
    }
...
class _ExportCenterSheetState extends State<_ExportCenterSheet> {
  ...
  Widget _scopeButton(
    ...
    return _Pressable(
      ...
      onTap: () => _popIfCurrent(context, _ExportChoice(_format, spec.scope)),
      borderRadius: BorderRadius.circular(22),
      child: Container(
        width: double.infinity,
        height: wide ? 76 : 72,
...
class _ExportService {
  ...
  static Future<void> _shareAiLedger(
    ...
    for (int rowIndex = 0; rowIndex < dataset.rows.length; rowIndex++) {
      ...
      output.writeln(
        'BEGIN_RECORD ${(rowIndex + 1).toString().padLeft(6, '0')}',
      );
